...
static void s_imageRGBtoYUV(Image_rgb *input, Image_yuv *output)
{
    ...
    for (int r = 0; r < h; r++)
    {
        for (int c = 0; c < w; c += 2)
		{
			...
			imRef(output, c, r).y = (UINT8) y;
			imRef(output, c, r).u = (UINT8) cb;
			imRef(output, c, r).v = (UINT8) cr;
		}
    }
};
...
static void s_imageYUVtoRGB(Image_yuv *input, Image_rgb *output)
{
    ...
    for (int r = 0; r < h; r++)
    {
		...
		{
			y = imRef(input, c, r).y;
			cb = imRef(input, c, r).u;
			cr = imRef(input, c, r).v;
			rc = y + ((           R_Cr[cr]) >> SCALE);
			g = y + ((G_Cb[cb] + G_Cr[cr]) >> SCALE);
			b = y + ((B_Cb[cb]           ) >> SCALE);
			imRef(output, c, r).r = (rc <= 0) ? 0 : (rc >= 255) ? 255 : rc;
			imRef(output, c, r).g = (g <= 0) ? 0 : (g >= 255) ? 255 : g;
			imRef(output, c, r).b = (b <= 0) ? 0 : (b >= 255) ? 255 : b;
        }
	}
};
                                  ...
                                  , float min, float max)
{
    ...
    float scale = (float)UINT8_MAX / (max - min);
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            uint8_t val = (uint8_t)((imRef(input, x, y) - min) * scale);
            imRef(output, x, y) = bound(val, (uint8_t)0, (uint8_t)UINT8_MAX);
        }
    }
};
...
{
    float min, max;
    float_min_max(input, &min, &max);
    s_imageFLOATtoUINT8_T(input, output, min, max);
};
...
static void s_imageLONGtoUINT8_T(Image_long *input, Image *output, long min, long max)
{
    ...
    float scale = (float)UINT8_MAX / (float)(max - min);
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
...
{
    long min, max;
    long_min_max(input, &min, &max);
    s_imageLONGtoUINT8_T(input, output, min, max);
};
...
{
    short min, max;
    short_min_max(input, &min, &max);
    s_imageSHORTtoUINT8_T(input, output, min, max);
};
...
{
    s_imageFLOATtoUINT8_T2(input, output);
};

void imageUINT8_TtoLONG(Image *input, Image_long *output)
{
    s_imageUINT8_TtoLONG(input, output);
};
...
{
    s_imageLONGtoUINT8_T2(input, output);
};
...
{
    s_imageSHORTtoUINT8_T2(input, output);
};
